...
//  Updated by Meysam Firouzi on 2025-08-19
//
...
class ConfigurationViewController: NSViewController, NSTextFieldDelegate {
    ...
    // Shared folder UI
    var sharedLabel: NSTextField!
    var sharedFolderPathControl: NSPathControl!

    convenience init(virtualMachine: VirtualMachine) {
        self.init()
        self.virtualMachine = virtualMachine
        cpuCounts = Array(1...ProcessInfo.processInfo.activeProcessorCount)
        memories = (1...(ProcessInfo.processInfo.physicalMemory >> 30)).map { $0 << 30 }
    }
    ...
    override func loadView() {
        ...
        memorySlider = LabeledSlider(labels: memories.map { formatter.string(fromByteCount: Int64($0)) })
        ...
        let screenMultiplicationLabel = NSTextField(labelWithString: "×")
        ...
        let innerScreenStackView = NSStackView(fixedSizeViews: [screenWidthTextField, screenMultiplicationLabel, screenHeightTextField, screenScaleCheckbox])
        ...
        // Shared folder
        sharedLabel = NSTextField(labelWithString: "Shared folder:")
        sharedFolderPathControl = NSPathControl()
        sharedFolderPathControl.target = self
        sharedFolderPathControl.action = #selector(sharedFolderChanged(_:))
        sharedFolderPathControl.isEditable = true
        sharedFolderPathControl.pathStyle = .popUp
        if let path = virtualMachine.metadata.configuration?.sharedDirectoryPath {
            sharedFolderPathControl.url = URL(fileURLWithPath: path, isDirectory: true)
        }
        let sharedStackView = NSStackView(fixedSizeViews: [sharedLabel, sharedFolderPathControl])
        sharedStackView.alignment = .firstBaseline
    ...
    @IBAction func sharedFolderChanged(_ sender: NSPathControl) {
        validateUI()
    }
    ...
    @IBAction func save(_ sender: NSButton) throws {
        virtualMachine.metadata.configuration = Configuration(
            ...
            sharedDirectoryPath: sharedFolderPathControl.url?.path
        )
